...
window.fbMethods.onAuthStateChanged(window.fbAuth, async (user) => {
    ...
    if (user) {
        ...
        // สำคัญ: บล็อกการเซฟชั่วคราว และล้างข้อมูลหน้าจอรอโหลดจากบัญชีนี้
        isCloudDataLoaded = false; 
        installments = []; 
        renderInstallments(); 

        await loadDataFromCloud(); // โหลดข้อมูลของบัญชีนี้มาใส่
    } else {
        // --- กรณีไม่ได้ล็อกอิน (Logout) ---
        currentUser = null;
        installments = []; // ล้างข้อมูลในตัวแปร
        localStorage.removeItem('data_v1'); // ล้างข้อมูลในเครื่องทิ้งทันที
        ...
        renderInstallments(); // หน้าจอจะว่างเปล่า
    }
});
// เรียกใช้งาน
initApp();
...
window.executeLogout = function() {
    window.fbMethods.signOut(window.fbAuth).then(() => {
        ...
        currentUser = null;
        installments = [];
        localStorage.removeItem('data_v1');
        isCloudDataLoaded = true;
        
        renderInstallments();
        closeLogoutModal();
        console.log("Logged Out & Cleared Data");
    });
}
...
async function loadDataFromCloud() {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            // ดึงข้อมูลมาใส่ในตัวแปรหลัก
            installments = Array.isArray(data) ? data : (data ? Object.values(data) : []);
            console.log("✅ โหลดข้อมูลของบัญชี " + currentUser.displayName + " สำเร็จ");
        } else {
            // ถ้าเป็นบัญชีใหม่กริ๊บ ไม่มีข้อมูลใน Cloud
            installments = [];
            console.log("🆕 บัญชีใหม่: เริ่มต้นข้อมูลว่างเปล่า");
        }
        
        isCloudDataLoaded = true; // ปลดล็อกให้เซฟได้
        renderInstallments();    // วาดหน้าจอ
        
    } catch (error) {
        console.error("❌ Load Error:", error);
        isCloudDataLoaded = true; // ปลดล็อกเพื่อให้ใช้งานต่อได้แม้โหลดพลาด
    }
}

// --- 3. ฟังก์ชันบันทึกข้อมูล (ปรับปรุงใหม่) ---
async function saveData() {
    // 1. ถ้าล็อกอินอยู่ แต่ข้อมูลจาก Cloud ยังโหลดมาไม่ถึง "ห้ามเซฟทับ" เด็ดขาด
    if (currentUser && !isCloudDataLoaded) {
        console.warn("⚠️ ระงับการเซฟ: กำลังรอโหลดข้อมูลจาก Cloud...");
        return;
    }

    // 2. เซฟลงเครื่อง (LocalStorage)
    localStorage.setItem('data_v1', JSON.stringify(installments));

    // 3. เซฟลง Cloud ถ้าล็อกอินอยู่
    if (currentUser) {
        try {
            ...
            console.log("☁️ บันทึกลง Cloud สำเร็จ");
        } catch (e) {
